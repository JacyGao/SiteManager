...
class Redportal_model extends Portal_model
{
    var $Age_Confirmation_Page = 0;

    function GetWallpapers($limit=9)
    {
        $out = $this->GetItems('Wallpapers',NULL,$limit);
        return $out;
    }
    ...
    function SendLoginURL($mobile, $country, $keyword)
    {
        ...
        $url = "http://{$this->Host->hostname}/redportal/login_url/{$country}/{$keyword}/?msisdn={$msisdn}&key=". urlencode($key);
    ...
    // resorting items according to ID, to latest content on top of the list
