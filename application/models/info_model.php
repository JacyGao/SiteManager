...
class Info_model extends MY_Model
{
    ...
    {
        $this->site_db->select("category");
        $this->site_db->group_by("category");
        $query = $this->site_db->get($this->tbl_items);
        ...
        foreach($query->result() as $rs)
        {
            switch($rs->category)
            {
                case "joke": $categories[ $rs->category ] = "Joke of the day"; break;
                case "bible": $categories[ $rs->category ] = "Daily bible quotes"; break;
                case "kiss": $categories[ $rs->category ] = "Daily kissing tips"; break;
                case "horo": $categories[ $rs->category ] = "Daily horoscopes"; break;
                case "quote": $categories[ $rs->category ] = "Quote of the day"; break;
                case "date": $categories[ $rs->category ] = "Daily dating tips"; break;
                case "love": $categories[ $rs->category ] = "Guru love tips"; break;

                default: $categories[ $rs->category ] = $rs->category; break;
            }
        }
        return $categories;
    }
