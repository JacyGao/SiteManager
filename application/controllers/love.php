...
class Love extends MY_Controller
{
    ...
    public function do_signup()
    {
        if( $_SERVER['REQUEST_METHOD'] == "POST")
        {
            ...
            if ($this->form_validation->run() == FALSE)
            {
                ...
                if( $this->agent->is_mobile() || strstr($this->agent->referrer(), "/love/index/") == true )
        ...
        if( $this->agent->is_mobile() )
            redirect($this->DocumentRoot."/love/signup_form/". $this->CountryKey ."/". $this->Keyword);
        else
            redirect($this->DocumentRoot."/love/signup/". $this->CountryKey ."/". $this->Keyword);
    }
    ...
    public function do_login(){
        if( $_SERVER['REQUEST_METHOD'] == "POST")
        {
            ...
            redirect($this->DocumentRoot."/love/login/". $this->CountryKey ."/". $this->Keyword);
    ...
    public function realStartLogin($mobile, $pin){
        ...
        if($credits < 1 ){
            ...
            redirect($this->DocumentRoot."/love/error/". $this->CountryKey ."/". $this->Keyword);
        }
        else
        $this->session->set_userdata('MSISDN', $mobile);
        $this->session->set_userdata('MSISDN_DETECTION_PERFORMED', 'FAKED');
        redirect($this->DocumentRoot."/love/index/". $this->CountryKey ."/". $this->Keyword);
    }
}